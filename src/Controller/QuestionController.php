...
class QuestionController extends AbstractController
{
    ...
    public function questionSearch(string $search='none',QuestionRepository $questionRep){
            $questions = $questionRep->findBySearch($search);

            return $this->json(json_encode($questions));
    }
    #[Route('/question/show/{id}', name: 'question_show')]
    public function show(Request $request, QuestionRepository $questionRep,int $id, EntityManagerInterface $em)
    {
        $question = $questionRep->getQuestionsWithCommentsAndAuthors($id);
        $options = [
            'question' => $question
        ];
        $user = $this->getUser();
    ...
    public function ratingQuestion(VoteRepository $voteRep, Request $request, Question $question, int $score, EntityManagerInterface $em)
    {
        $user = $this->getUser();
    ...
    public function ratingComment(VoteRepository $voteRep, Request $request, Comment $comment, int $score, EntityManagerInterface $em)
    {
        $user = $this->getUser();
